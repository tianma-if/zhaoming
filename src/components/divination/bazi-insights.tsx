          ...
          <div className="space-y-6">
            <div className="grid gap-4 rounded-xl border border-border bg-muted/20 p-5 min-[760px]:grid-cols-[minmax(0,1fr)_18rem]">
              <div className="space-y-2">
                ...
                <p className="text-2xl font-semibold">{view.daYun.startLabel}</p>
                <p className="text-muted-foreground">{view.daYun.startSolar}</p>
              </div>
              {view.daYun.current ? (
                <div className="rounded-lg border border-border bg-muted/45 p-4">
                  <p className="text-sm text-muted-foreground">当前大运</p>
                  <p className="mt-1 text-2xl font-semibold">
                    年龄范围： {view.daYun.current.ageRange}
                  </p>
                  <p className="mt-1 text-muted-foreground">
                    {view.daYun.current.tenGod} · {view.daYun.current.ganZhi}
                  </p>
                </div>
              ) : null}
            </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-[5rem_1fr] gap-x-4 gap-y-1 text-sm">
                      ...
                      <p>
                        ...
                        {isChildStage ? "依靠父母庇护" : daYunFeatures[period.tenGod] ?? "阶段特征需结合全盘判断"}
                      </p>
            ...
            <div className="space-y-3 rounded-xl bg-muted/20 p-5 text-sm leading-7">
              ...
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                ...
                {view.daYun.current ? (
                  <li>
                    当前大运（年龄范围：{view.daYun.current.ageRange}）是
                    {view.daYun.current.tenGod}阶段，适合结合现实目标调整节奏
                  </li>
                ) : null}
              </ul>
            </div>
