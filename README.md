...
- ☯️ **专业纯粹的排盘**：支持八字（大运、十神、神煞）及紫微斗数（4x4 环形星盘）排盘，界面极简、无任何弹窗广告。
- 🤖 **智能 AI 流式解盘**：通过流式对话与个性化判词，用理性的视角剖析命盘与命运选择。
- ⚡ **开箱即用的全栈架构**：基于 Next.js + Better Auth + Neon Postgres + Vercel AI SDK 构建，基础设施完整就绪，可直接自部署或作为开发模板。
- 💸 **极致的 Token 成本优化**：采用「确定性算法排盘（零成本） + 按需 AI 解读」双轨架构，完美规避大模型 API 账单爆炸风险。
